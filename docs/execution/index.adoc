...
| prebuild   | bdebstrap | Within setup phase
...
Runner scans for `<phase>*` hooks (eg `customize*`, `preimage05-do-something`) inside `hooks/` subdirectories at each location. It executes matching hooks (alphanumeric basename) in that order. Subdirectories aren’t traversed; the file extension is ignored.
...
|===
| Overlay              | Applied
| `customize.overlay`  | Before `bdebstrap` runs, positioned immediately ahead of that layer's own `customize-hooks`, so a layer's own hooks (eg an `enable-units` call) can depend on its own overlay content
...
`<layer-stem>.d/rootfs-overlay` and `<layer-stem>.d/overlay` are both accepted as compat spellings of `customize.overlay`, so an existing `rootfs-overlay` tree can be moved into the layer's `.d/` unchanged, or a layer can use the shorter `overlay` name where it doesn't need to say which phase applies it. Where a layer ships more than one, they are applied oldest spelling first (`rootfs-overlay`, then `overlay`, then `customize.overlay`), so the later spelling wins on any file more than one provides.
