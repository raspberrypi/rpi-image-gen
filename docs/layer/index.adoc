...
`${STEM}`: Name of the layer file without its extension

`${DOTD}`: The layer's optional companion directory, ie `${DIRECTORY}/${STEM}.d`, where it ships its hooks, overlays, etc.
...
- **Placeholders**: Support for dynamic values like `${DIRECTORY}`, `${FILENAME}`, `${STEM}`, `${NAME}`, `${VERSION}`, `${UUID}`, `${WORKDIR}` and `${DOTD}`
