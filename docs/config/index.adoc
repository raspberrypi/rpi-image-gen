...
[NOTE]
====
`trait:` (see <<section-trait>>) is the one exception to the rule above - it is never translated to `IGconf_*` variables at all.
====
...
Unlike every other section, `trait:` is **never** translated to `IGconf_<section>_*` variables. It's a reserved top-level key, passed through as-is and applied directly as a build-time override to the trait registry - taking precedence over anything declared by the registry itself or by a layer's `X-Env-Layer-Provides`.
...
`false` removes the named token (and any of its children, by prefix match) from the build's provider index, even if a layer in the build declares it. Any other value - `true` for a boolean trait, or an explicit typed value otherwise - injects the token, exactly as if a layer had declared it. Trait overrides merge across includes like any other section - the including file's entries win over an included file's on a shared token, and non-overlapping tokens from both are kept.

See link:../trait.adoc[trait.adoc] for the full trait registry reference, including token namespaces, hierarchy, and `has()`.
...
* **immediate**: Set the variable if it is currently unset (first-wins strategy). This is the default behaviour.
