...
The device image will be in `./work/image-deb13-arm64-min/deb13-arm64-min.img`
...
The latest documentation is also available at https://raspberrypi.github.io/rpi-image-gen. Run `rpi-image-gen docs` to browse docs matching your local version. This starts a web server on port 3142, or on the port given as an argument, eg `rpi-image-gen docs 9000`. Point your browser to the URL printed on startup.

=== Examples

See the link:./examples[examples directory] for tips and help.

=== Directory Structure

* `bin/` - Common executable files and utilities
* `builtin/` - Built-in hooks and helper tooling
* `config/` - Configuration files
* `contrib/` - Optional, community-maintained extensions and tooling outside the supported core
